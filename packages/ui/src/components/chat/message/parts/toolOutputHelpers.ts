...
export const getEffectiveToolOutput = (
    ...
    if (status === 'completed' && typeof stateOutput === 'string') {
        return stateOutput;
    }
    if (tool === 'bash' && status === 'running' && typeof metadataOutput === 'string' && metadataOutput.length > 0) {
        return renderTerminalOutput(metadataOutput);
    }
    return typeof stateOutput === 'string' ? stateOutput : undefined;
};
